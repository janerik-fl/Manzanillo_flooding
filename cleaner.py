...
- Strips unprintable / emoji characters (⏩, etc.).
- Writes cleaned .xlsx files (UTF-8 safe for Excel).
"""
...
# CONFIGURATION
# --------------------------------------------------------------------
input_folder = Path("input_raw_data")     # Folder containing raw .csv files
output_folder = Path("output_cleaned_data")  # Folder for cleaned .xlsx outputs
...
def strip_non_ascii(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def clean_text(x):
        if isinstance(x, str):
            # Remove emojis, arrows, control chars, etc.
            cleaned = re.sub(r"[^\x00-\x7F]+", "", x)
            return cleaned
        return x
...
def clean_file(path: Path) -> DataFrame:
    ...
    # Reorder (guarantees DataFrame return type)
...
def main() -> None:
    print(f" Cleaning all CSVs in: {input_folder.resolve()}")
    for file in input_folder.glob("*.csv"):
        print(f"Processing {file.name} ...")
        try:
            cleaned = clean_file(file)
            cleaned = strip_non_ascii(cleaned)
